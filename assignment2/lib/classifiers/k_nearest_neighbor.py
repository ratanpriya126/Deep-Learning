...
class KNearestNeighbor(object):
  ...
  def compute_distances_two_loops(self, X):
    ...
    for i in xrange(num_test):
        for j in xrange(num_train):
            differences =  X[i, :] - self.X_train[j,:]
            #print differences
            squareDifferences = np.square(differences)
            #print squareDifferences
            sumSquareDifferences = np.sum(squareDifferences)
  ...
  def compute_distances_one_loop(self, X):
    ...
    #print testVector.shape
        ...
        testDifference = np.square(testDifference)
  ...
  def predict_labels(self, dists, k=1):
    ...
    #print testArgsorted
        closest_y = np.take(self.y_train, testArgsorted)[:k]#self.y_train[testArgsorted]
        (uniqClass, count) = np.unique(closest_y, return_counts=True)
        y_pred[i] = uniqClass[np.argmax(count)]
      #########################################################################
      # TODO:                                                                 #
